  ...
  public computeCell$(
    ...
    .pipe(
      ...
      rxo.map((o: any) => {
        ...
        o.rows.map((x: any) =>
          sumInverseDistances += Math.pow(x.d, -this.power));

        // Calculate the weight of each point
        o.rows.map((x: any) =>
          weight.push(Math.pow(x.d, -this.power) / sumInverseDistances));

        // Calculate the final height
        for (let i = 0; i<o.rowCount; i++) h += weight[i]*o.rows[i].h;
        h = round(h, this.round);
