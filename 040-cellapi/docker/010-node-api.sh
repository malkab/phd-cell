...
# -----------------------------------------------------------------
#
# Runs an interactive Node session to run the API.
#
# -----------------------------------------------------------------
#
...
JUPYTER=false
# Container name.
CONTAINER_NAME=cellapi-node-dev
# Container host name. Incompatible with NETWORK=container:XXX.
CONTAINER_HOST_NAME=cellapi-node-dev
...
eval  $DOCKER_COMMAND \
        ...
        -e "MLKC_CELL_VERSION=${MLKC_CELL_VERSION}" \
        -e "MLKC_CELL_INSTANCE_GENERATION=${MLKC_CELL_INSTANCE_GENERATION}" \
        -e "NODE_ENV"=${NODE_ENV} \
        -e "MLKC_CELL_VERSION"=${MLKC_CELL_VERSION} \
        -e "MLKC_CELL_INSTANCE_GENERATION"=${MLKC_CELL_INSTANCE_GENERATION} \
        -e "NODE_MEMORY"=${NODE_MEMORY} \
        -e "MLKC_CELL_DB_HOST"=${MLKC_CELL_DB_HOST} \
        -e "MLKC_CELL_DB_INNER_PORT"=${MLKC_CELL_DB_INNER_PORT} \
        -e "MLKC_CELL_DB_USER"=${MLKC_CELL_DB_USER} \
        -e "MLKC_CELL_DB_PASSWORD"=${MLKC_CELL_DB_PASSWORD} \
        -e "MLKC_CELL_DB_MAXPOOLSIZE"=${MLKC_CELL_DB_MAXPOOLSIZE} \
        -e "MLKC_CELL_DB_MINPOOLSIZE"=${MLKC_CELL_DB_MINPOOLSIZE} \
        -e "MLKC_CELL_DB_IDLETIMEOUTMILLIS"=${MLKC_CELL_DB_IDLETIMEOUTMILLIS} \
        -e "MLKC_CELL_REDIS_URL"=${MLKC_CELL_REDIS_URL} \
        -e "MLKC_CELL_REDIS_PASSWORD"=${MLKC_CELL_REDIS_PASSWORD} \
        $NETWORK \
        $CONTAINER_NAME \
        $CONTAINER_HOST_NAME \
        $X11 \
